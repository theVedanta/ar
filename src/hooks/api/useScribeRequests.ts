...
export const useScribeRequests = ({
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/scribe-requests?${params.toString()}`);
      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.error || 'Failed to fetch scribe requests');
      }
  ...
  const createRequest = useCallback(async (requestData: {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to create scribe request');
      }
  ...
  const approveRequest = useCallback(async (requestId: string, userId: string) => {
    setError(null);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to approve scribe request');
      }
      ...
      setRequests(prev =>
        prev.map(request =>
          request.id === requestId
            ? { ...request, status: 'approved', approvedBy: userId, approvedAt: new Date() }
            : request
        )
      );
    } catch (err) {
  ...
  const rejectRequest = useCallback(async (requestId: string, userId: string, reason: string) => {
    setError(null);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to reject scribe request');
      }
