...
import { Users, BookOpen, CheckCircle, AlertTriangle, Star, Search, Filter } from "lucide-react"
...
const allMatches = [
  {
    id: 1,
    student: { name: "Arjun Mehta", class: "12th", subjects: ["Math", "Physics"] },
  ...
  {
    id: 2,
    student: { name: "Sneha Gupta", class: "11th", subjects: ["Chemistry", "Biology"] },
  ...
  {
    id: 3,
    student: { name: "Rohit Singh", class: "12th", subjects: ["English", "History"] },
    scribe: null,
    examDate: "2024-12-22",
    status: "unmatched",
    matchScore: 0,
...
export default function AdminDashboard() {
  ...
  const filteredMatches = allMatches.filter((match) => {
    const matchesSearch =
      ...
      (match.scribe?.name.toLowerCase().includes(searchTerm.toLowerCase()) ?? false)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total Scribes</CardTitle>
              <BookOpen className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{stats.totalScribes}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Tabs defaultValue="matches" className="space-y-6">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle className="text-lg">
                          {match.student.name} → {match.scribe?.name || "Unassigned"}
                        </CardTitle>
                        <CardDescription>
                          {match.student.class} • {match.student.subjects.join(", ")} • Exam: {match.examDate}
                        </CardDescription>
                      </div>
                      ...
                      <div className="space-y-1">
                        {match.scribe && (
                          <div className="flex items-center gap-2 text-sm text-gray-600">
                            ...
                            <span>{match.scribe.rating}</span>
                            <span>•</span>
                            <span>{match.scribe.experience}</span>
                          </div>
                        )}
                      </div>
                      <div className="flex gap-2">
                        {match.status === "unmatched" && (
                          <Button onClick={() => handleManualAssign(match.id)}>Assign Manually</Button>
                        )}
                        {match.status === "pending" && <Button variant="outline">Approve Match</Button>}
                      </div>
                    </div>
                  </CardContent>
                </Card>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-medium">
                            {feedback.studentName} → {feedback.scribeName}
                          </h4>
          ...
          <TabsContent value="reports" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Top Performing Scribes</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex justify-between">
